...
router.get('/', autenticado, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id_usuario, nome, email, telefone, tipo, data_cadastro FROM usuarios'
    );
...
router.post(
  ...
  [
    ...
    body('tipo').optional().isIn(['usuario', 'admin']).withMessage('Tipo inválido'),
  ],
  async (req, res) => {
    ...
    try {
      ...
      const result = await db.query(
        ...
        [nome, email, hashSenha, telefone || null, tipo || 'usuario']
      );
...
router.delete('/:id', autenticado, async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const result = await db.query(
      'DELETE FROM usuarios WHERE id_usuario = $1 RETURNING id_usuario, nome',
      [id]
    );
